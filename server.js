...
// ── STATIC FILES — must come BEFORE API routes that use '*' ───────────────
// FIX 1: Moved static middleware up so index.html is served correctly
...
// FIX 2: Scope the wildcard to non-API paths to avoid swallowing API 404s
app.get(/^(?!\/api).*$/, (_req, res) => {
  const indexPath = path.join(__dirname, 'public', 'index.html');
  res.sendFile(indexPath, (err) => {
    if (err) {
      console.error('Error serving index.html:', err.message);
      // FIX 3: Don't call res.status() after res.sendFile() has already
      //        written headers — check if headers were sent first
      if (!res.headersSent) {
...
// ── ERROR HANDLER ─────────────────────────────────────────────────────────
// FIX 4: Error handler must be defined AFTER all routes
...
// ── START ─────────────────────────────────────────────────────────────────
// FIX 5: Bind to '0.0.0.0' explicitly so Railway/Render can route to it
